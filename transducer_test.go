...
func testTransducer(regexp string, test func(*transducer)) {
...
func TestTransducer(t *testing.T) {
	testTransducer(`(<a,>+<b,>)*.<a,>.<b,>.<b,>`, func(tr *transducer) {
		state1 := tr.root
...
func TestSameInputTape(t *testing.T) {
	testTransducer(`<a,b>+<a,c>+<a,d>`, func(tr *transducer) {
		state1 := tr.root
		assert.Equal(t, 1, state1.index)
		...
		next := state1.next['a']
		assert.True(t,
			next[0].state == next[1].state && next[1].state == next[2].state)
...
func TestMulticharInputTransducer(t *testing.T) {
	testTransducer(`<abc,xy>+<aca,zz>`, func(tr *transducer) {
		state1 := tr.root
		assert.Equal(t, 1, state1.index)
